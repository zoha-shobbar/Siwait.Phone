...
public static partial class IConfigurationBuilderExtensions
{
    ...
    public static void AddClientConfigurations(this IConfigurationBuilder builder, string clientEntryAssemblyName)
    {
        ...
        var sharedAssembly = Assembly.Load("Siwait.Phone.Shared");

        configBuilder.AddJsonStream(sharedAssembly.GetManifestResourceStream("Siwait.Phone.Shared.appsettings.json")!);

        var envSharedAppSettings = sharedAssembly.GetManifestResourceStream($"Siwait.Phone.Shared.appsettings.{AppEnvironment.Current}.json");
        if (envSharedAppSettings != null)
        {
            configBuilder.AddJsonStream(envSharedAppSettings);
        }

        var clientCoreAssembly = AppDomain.CurrentDomain.GetAssemblies().Single(asm => asm.GetName()?.Name is "Siwait.Phone.Client.Core");

        configBuilder.AddJsonStream(clientCoreAssembly.GetManifestResourceStream("Siwait.Phone.Client.Core.appsettings.json")!);

        var envClientCoreAppSettings = clientCoreAssembly.GetManifestResourceStream($"Siwait.Phone.Client.Core.appsettings.{AppEnvironment.Current}.json");
